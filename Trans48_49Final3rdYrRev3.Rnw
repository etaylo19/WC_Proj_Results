...
\title{An Analysis of Workplace Injury and Job Tenure in the Transportation and Warehousing Sector Using Tennessee Workers' Compensation Claims from 2014-2016}
...
################################################################
INDUSTRY_TYPE <- 'transportation'         #### will change
MEDIAN_TENURE <- 4.4
...
\item Lifting and strains\footnote{We differentiate between strain as a cause and strain as a type. The former is an overexertion and the latter can be defined as an overstretching of some part of the musculature.} were the leading sources/causes of injury in \Sexpr{INDUSTRY_TYPE}. 
...
\item The lower back and shoulders were the most frequently injured body parts. Lifting (back) and cumulative stresses (shoulders) are likely factors.
Recommendation: As above, provide training on lifting best practices and modify work procedures to reduce physical stresses.
\item About eight percent of all claims required an emergency room visit. Recommendation:  Have protocols in place to ensure injured workers requiring emergency care receive rapid and safe transport.
\item Transportation had more fatalities than any other NAICS sector and most were motor vehicle accidents. Thirty-six percent of those fatally injured were in their first year with their employer.
Recommendation: Ensure drivers adhere to mandatory rest periods and other limits on driving time. Conduct pre-employment and periodic motor vehicle record checks on all drivers of company vehicles.
\item Workers were especially vulnerable to injury during their first year. Forty percent of all claims in \Sexpr{INDUSTRY_TYPE} were made by such workers. Smaller employers had even higher first-year injury proportions. 
...
\item As a proportion of all injuries, non-fatal severe injuries were approximately ten percent. Almost half occurred to first-year employees.
...
There are at least two other important limitations of the data. First, older workers and those with longer tenure may not be exposed to the same hazards as new workers. Second, the claim data can be used to profile only injured workers. Though there is some public information on workforce age distribution and median tenure, we know little about other characteristics of workers not injured.
...
The data show that size of the employer may make a difference for injuries to new employees. Table \ref{tab:FirmSz} indicates the proportion of first-year injuries begins to decrease as the firm size reaches 20 employees and decreases significantly at 100 employees. Larger firms are usually thought to expend more effort in training new employees. Firms with 10-19 employees had first-year employees reporting injuries in a greater proportion than other groups. Firms with less than 10 employees had slightly lower numbers than the groups spanning 10-49. 
...
For type of injury (e.g. sprain, cut, etc.) and cause of injury (e.g. fall, slip, exertion, etc.), see Tables \ref{tab:InjType} and \ref{tab:InjCause}. In the former, first-year rankings closely approximate the ranking structure for other periods. In the latter, there is some rank variation. Not surprisingly, sprains, contusions, strains, and lacerations rank among the most frequent injury types and the lower back was the most frequent body part injured. It is interesting that lifting was one of the top two causes. One possible interpretation is that workers lifting heavy objects resulted in strains/overexertions that became musculoskeletal injuries to the lower back.
...
Most claims in the data contain an associated injury narrative and the word cloud in Figure \ref{fig:WordCloud} features the most common words found in the narratives. The cloud reinforces the Table \ref{tab:BodyPart} results by showing `back' as the most prominent. The nouns `package' and `box' appear as does the verb `lift'. All these things suggest that lifting injuries are a major concern in transportation.

\section{Injury Severity}

To explore, non-fatal injury severity, Table \ref{tab:Treatment} shows the various initial treatment modes given in the claims data. Using these modes as a proxy for severity, we assumed those claims associated with future major medical, hospitalization, and emergency evaluation represent the most severe injuries. For hospitalization and emergency, first-year injury proportions approximated or exceeded those of the first-year baseline of \Sexpr{ round(cumsum(counts_ten / sum(counts_ten) )[2] , 2) }.

<<Treatment, echo=FALSE, fig.pos='!h'>>=
# Table 7
# See code for 'TrtmtGp' table above
# match codes to types(alpha)
Treatment_final <- Treatment_final[c(3,7,6,1,5,2,4) , c(1,3,2,4)]
...
As stated earlier, job tenure is unknown for \Sexpr{round(100 * sum(is.na(IndTable$tenure_yrs))/nrow(IndTable), 0)} percent of \Sexpr{INDUSTRY_TYPE} sector claims. For claims containing such information, Table \ref{tab:Treatment} indicates that first-year employees received almost half of all hospitalization and emergency evaluation treatments.



For fatal injuries, there were \Sexpr{sum(!is.na(IndTable$DTE_OF_DTH))} incidents during the study period. In this sector, \Sexpr{length(which(IndTable$DTE_OF_DTH != 'NA' & IndTable$tenure_yrs <= 1))} (\Sexpr{ round ( 100 * length(which(IndTable$DTE_OF_DTH != 'NA' & IndTable$tenure_yrs <= 1)) / sum(!is.na(IndTable$DTE_OF_DTH)) , 0)} percent) fatalities had employer service of less than one year. Not surprisingly, Table \ref{tab:InjTypeFat} indicates that vehicle-related fatalities predominate.
...
\item Among two-digit NAICS industries, transportation was above average for WC injury frequency  with a rate of \Sexpr{round(InjComp[(match(INDUSTRY_TYPE, InjComp$Sector_Description)), 4] , 1)} per 100 FTE (extrapolated rate is 5.0 per 100 FTE).
...
\item Strains of the musculature were the most common type of injury and happened at almost one and one-half times the rate of the second most common (contusions). The lower back was the most common body part to be injured with shoulders second.
...
\item The job injury-tenure relationship varied with the size of employer. Firms with less than 100 employees had almost half of their injuries suffered by first-year employees vs. less than 40 percent for those with more than 100 employees.

\item First-year injuries had rank orders for injury type almost identical to those of the set of all injuries. Injury cause was more variable between the groups but lifting and strains were first or second in both groups.

\item Workers older than 55 had lower overall rates of injury than their younger peers.

\end{enumerate}

Further, \Sexpr{INDUSTRY_TYPE} is different than most industries because of more exposure to moving vehicle accidents. These accidents perenially cause more workplace fatalities than any other type. Further, the extreme physical demands of loading/unloading cargo in addition to extended driving spells leave newly hired workers vulnerable in the early portion of their job tenure with an employer. Employees working in smaller firms are more at risk. The industry should develop practices (e.g. new employee onboarding, mentorship, etc.) that target these at-risk workers.
